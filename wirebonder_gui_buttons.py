...
class InfoButton(QPushButton):
    def __init__(self, width, height, w_width, parent = None):
        super().__init__(parent)
        ...
        string = ("The frontside page shows the frontside of the module with each cell numbered, as well as the notches (negative numbers) and guardrail bonds"
                ...
                "3: red/missing three bonds. Each button may be right clicked, which will give it one of three ground flags: 0: no outline/does not need to be grounded, 1: outline/needs to be grounded, "
                "2: black fill/has been grounded. All color based flags may be set to blue and all grounded flags to normal with the \"Set to Nominal\" button."
                "<br><br>The \"Reset to last saved version\" resets the code to the most recent version in the database, which will lose all changes made since then. "
                "The pull testing page has text boxes for optional pull testing information.<br><a href=\"https://docs.google.com/presentation/d/1mPAw4cvQ1-pvM9ypXFmarl_0cmfvkjjfg1GeegAXgzc/edit?usp=sharing\">More information</a>")
        self.box = QLabel(string, self.parent)
        self.box.setWordWrap(True)
        self.box.setOpenExternalLinks(True)
        self.closebutton = GreyButton("Close", 75, 25, self.parent)
        self.closebutton.hide()
        self.box.hide()

    def show_info(self):
        self.box.setGeometry(self.w_width/2 - 550/2,100, 550, 325)
        ...
        self.closebutton.setGeometry(int((self.w_width/2)-75/2),390,75,25)
