...
def find_to_revisit():
    ...
    for module in all_modules:
        ...
        if check['exists']:
            read_query = f"""SELECT cell_no, bond_type
            FROM front_wirebond
            WHERE module_name = '{module['module_name']}'
            ...
            if "N" in front_res['bond_type']:
                bad_modules.append(module['module_name'])
                #don't want to add module name twice, so don't run the rest
                continue
            ...
            if check['exists']:
                ...
                for cell in ground:
                    #if a cell is in test, it should also be in front_wirebonder and marked as successfully grounded
                    if not (cell in front_res['cell_no'] and front_res['bond_type'][front_res['cell_no'].index(cell)] == "G"):
                        to_add = True
                if to_add:
                    bad_modules.append(module['module_name'])
    return(bad_modules)
...
def read_front_db(modname, df_pad_map):
    ...
    front_wirebond_info = {'technician': None, 'comment': None, 'wedge_id':'','spool_batch':''}
...
def read_back_db(modname, df_backside_mbites_pos):
    ...
    back_wirebond_info = {'technician': '', 'comment': '', 'wedge_id':'','spool_batch':''}
