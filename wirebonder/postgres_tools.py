...
def read_front_db(modname, df_pad_map):
    ...
    if not check['exists']:
        ...
        for index, row in df_pad_map.iterrows():
            if index >198: #these are pads that don't correspond to pads on the board
                continue
            if int(index) in ground:
                is_grounded = 1
            else:
                is_grounded = 0
            df_front_states.loc[index] = {"state": 0,"grounded":is_grounded}
    else:
        #read from front_wirebond
        ...
        for index, row in df_pad_map.iterrows():
            ...
            if int(index) in front_res['cell_no']:
                ...
                state = is_grounded = 0
            df_front_states.loc[index] = {'state' : state, 'grounded' : is_grounded}

    return {"df_front_states" : df_front_states,  "front_encaps_info":front_encaps_info,
...
def read_back_db(modname, df_backside_mbites_pos):
    ...
    if not check['exists']:
        for index, row in df_backside_mbites_pos.iterrows():
            df_back_states.loc[index] = {"state": 0,"grounded":0}
    else:
        #read from front_wirebond

        read_query = f"""SELECT mbite_no, bond_count_for_mbite, technician, comment
        FROM back_wirebond
        WHERE module_name = '{modname}'
        ORDER BY date_bond DESC, time_bond DESC LIMIT 1;"""
        ...
        for index, row in df_backside_mbites_pos.iterrows():
            if int(index) in back_wirebond_info["mbite_no"]:
                state = 3-back_wirebond_info['bond_count_for_mbite'][back_wirebond_info['mbite_no'].index(int(index))]
            else:
                state = 0
            df_back_states.loc[index] = {"state": state,"grounded":0}

    return { "df_back_states": df_back_states, "back_encaps_info" : back_encaps_info,
