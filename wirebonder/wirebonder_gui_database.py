...
class FrontPage(QMainWindow):
    def __init__(self, modname, df_pad_map, df_backside_mbites_pos, df_pad_to_channel, info_dict):
        ...
        for state in self.state_counter:
            ...
            lab.setGeometry(20,10 + state * 20,150, 20)
        ...
        lab4.setGeometry(20,260,150,50)
        self.comments = QTextEdit(self.widget)
        self.comments.setGeometry(20, 300, 150, 150)
        ...
        for index,row0 in self.df_pad_map.iterrows():
            ...
            if row1['Channeltype'] == 0 and index > -1:
                ...
                #set pad position
                pad.setGeometry(int(float(row0["xposition"]*scaling_factor) + scroll_width/2 ),int(float(row0["yposition"]*-1*scaling_factor + y_offset+ w_height/2)), int(pad.radius*2), int(pad.radius*2))
            #create half hexagon cells
                ...
                pad.setGeometry(int(float(row0["xposition"]*scaling_factor) + scroll_width/2 ),int(float(row0["yposition"]*-1*scaling_factor + y_offset+ w_height/2)), int(pad.radius), int(pad.radius*2))
            elif row1['Channeltype'] == 3 and index > -1:
                pad = HalfHexWithButtons(self.buttons, self.state_counter, self.state_counter_labels, self.state_button_labels,row2['state'],row2['grounded'], hex_length, str(padnumber), [-hex_length/2,0],
                    str(row1['Channel']), int(row1['Channelpos']), '#d1dbe8',row1['Channeltype'],  self.widget)
                pad.setGeometry(int(float(row0["xposition"]*scaling_factor) + scroll_width/2 +pad.radius),int(float(row0["yposition"]*-1*scaling_factor + y_offset+ w_height/2)), int(pad.radius), int(pad.radius*2))
            #create calibration channels
                ...
                pad.setGeometry(int(float(row0["xposition"]*scaling_factor) + scroll_width/2 + hex_length*2/3),int(float(row0["yposition"]*-1*scaling_factor + y_offset+w_height/2+hex_length*2/3)), int(pad.radius*2), int(pad.radius*2))
            ...
            #create mousebites and guardrails
                ...
                pad.setGeometry(int(float(row0["xposition"])*scaling_factor + scroll_width/2 + scaling_factor*0.25),int(float(row0["yposition"]*-1*scaling_factor + y_offset + w_height/2 + scaling_factor*0.25)),
...
class BackPage(QMainWindow):
    def __init__(self, modname, df_pad_map, df_backside_mbites_pos, df_pad_to_channel, info_dict):
        ...
        #make label of state counter
        for state in self.state_counter:
            lab = QLabel(f"{state}: {self.state_counter[state]}", self.widget)
        ...
        for index,row0 in self.df_pad_map.iterrows():
            ...
            if self.df_pad_to_channel.loc[padnumber]['Channeltype'] == 0 and index > -1:
                ...
                pad.setGeometry(int(float(row0["xposition"]*-1*scaling_factor) + scroll_width/2 ),int(float(row0["yposition"]*-1*scaling_factor + w_height/2 + y_offset)), int(pad.radius*2), int(pad.radius*2))
            #half hexagons
        ...
        for index,row in self.df_backside_mbites_pos.iterrows():
            ...
            pad.setGeometry(int(float(row["xposition"])*-1*scaling_factor + scroll_width/2+ scaling_factor*0.25),int(float(row["yposition"]*-1*scaling_factor + w_height/2+ y_offset + scaling_factor*0.3)), int(pad.radius*2), int(pad.radius*2))
            self.buttons[str(padnumber)] = pad

        pad2 = GreyCircle(13, 0, 0, self.widget)
        pad2.setGeometry(int(scroll_width/2 +pad.radius*2),int(w_height/2+y_offset), int(pad.radius*2), int(pad.radius*2))
        ...
        pad3.setGeometry(int(scroll_width/2 +pad.radius*2),int(w_height/2+y_offset - diff), int(pad.radius*2), int(pad.radius*2))
