...
class FrontPage(QMainWindow):
    def __init__(self, modname, df_pad_map, df_backside_mbites_pos, df_pad_to_channel, info_dict):
        ...
        for state in self.state_counter:
            lab = QLabel(f"{state} missing bonds: {self.state_counter[state]}", self)
        ...
        nominal_button.show()
        info_label = QLabel("<a href=\"https://github.com/nkalliney1/wirebonder_gui/blob/main/README.md\">Help",self)
        info_label.setOpenExternalLinks(True)
        info_label.setGeometry(w_width-40, 110, 100, 25)
