...
from wirebonder_gui_buttons import Hex, HexWithButtons, WedgeButton, GreyButton, SetToNominal, ResetButton, InfoButton, SwitchPageButton, SaveButton, ResetButton2, HalfHexWithButtons, HalfHex
    ...
    def __init__(self, modname, df_pad_map, df_backside_mbites_pos, df_pad_to_channel, info_dict):
        ...
        for index,row0 in self.df_pad_map.iterrows():
            ...
            if row1['Channeltype'] == 0 and index > -1:
                pad_after = False
                pad_before = False
                if (index < (len(self.df_pad_map) - num_mousebites-1)):
                ...
                pad.setGeometry(int(float(row0["xposition"]*scaling_factor) + w_width/2 ),int(float(row0["yposition"]*-1*scaling_factor + y_offset+ w_height/2)), int(pad.radius*2), int(pad.radius*2))
            elif self.df_pad_to_channel.loc[padnumber]['Channeltype'] == 1 and padnumber > 0: #calibration channel pads
                ...
                pad.raise_()
            else: #padnumber < 0 (i.e. mousebites)
...
class MainWindow4(QMainWindow):
    ...
    def begin_program(self):
        modname = self.combobox.currentText()+"-"+self.modid.text()
        hexaboard_type = modname[5] + modname[7]
        global hex_length, y_offset, num_mousebites
        if modname[5] == "L":
            hex_length = 38
        elif modname[5] == "H":
            hex_length = 25
        ...
        info_dict = read_from_db(modname, self.df_pad_map, self.df_backside_mbites_pos)
        widget = QStackedWidget(self)
        mainWindow = MainWindow(modname, self.df_pad_map, self.df_backside_mbites_pos, self.df_pad_to_channel, info_dict)
        widget.addWidget(mainWindow)
        mainWindow3 = MainWindow3(modname, info_dict)
        widget.addWidget(mainWindow3)
        mainWindow2 = MainWindow2(modname,self.df_pad_map, self.df_backside_mbites_pos, self.df_pad_to_channel,info_dict)
        widget.addWidget(mainWindow2)
        widget.setCurrentWidget(mainWindow)
        #sets window size (edit to make full screen)
        widget.setGeometry(0, 25, w_width, w_height)
        widget.show()
        ...
        label.setGeometry(w_width-90-10-225, 0, 225, 25)
        label.show()
        save_button = SaveButton(mainWindow, mainWindow2, mainWindow3, modname, label, 90, 25, "Save", self)
        save_button.setGeometry(w_width-save_button.width-10, 0, save_button.width, save_button.height)
        save_button.show()
        label2 = QLabel(modname, self)
        label2.setGeometry(int(w_width/2), 0, 150, 25)
        label2.show()
